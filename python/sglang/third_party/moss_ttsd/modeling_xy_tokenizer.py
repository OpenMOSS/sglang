...
from sglang.third_party.moss_ttsd.feature_extraction_xy_tokenizer import (
    ExtractorIterator,
...
class SinusoidalPositionalEmbedding(nn.Module):
    """Sinusoidal positional embedding as a reusable nn.Module.

    Note: This intentionally provides a backward-compatible state_dict loader for
    older checkpoints where the parent module had a tensor buffer named
    `positional_embedding` (e.g. `semantic_encoder.positional_embedding`).
    """

    def __init__(self, max_positions: int, dim: int, max_timescale: int = 10000):
        super().__init__()
        self.max_positions = int(max_positions)
        self.dim = int(dim)
        self.max_timescale = int(max_timescale)
        self.register_buffer(
            "weight",
            sinusoids(self.max_positions, self.dim, max_timescale=self.max_timescale),
        )

    def forward(
        self, seq_len: int, dtype: Optional[torch.dtype] = None
    ) -> torch.Tensor:
        seq_len = int(seq_len)
        emb = self.weight[:seq_len] if seq_len < self.weight.shape[0] else self.weight
        return emb.to(dtype=dtype) if dtype is not None else emb
    ...
    def _load_from_state_dict(
        ...
        # Backward-compat: old checkpoints stored a Tensor buffer at key
        # `<prefix_without_trailing_dot>` (e.g. `...positional_embedding`).
        old_key = prefix[:-1]
    ...
    def forward(
        ...
        if seq_len is not None:
            ...
            attn_mask = self._create_attention_mask(
                seq_len, tgt_len, hidden_states.device, attn_weights.dtype
            )
            attn_weights = (
            ...
            if attention_mask.dim() == 2:
                attention_mask = _prepare_4d_attention_mask(
                    attention_mask, hidden_states.dtype, tgt_len
                )
...
class XYTokenizerEncoder(nn.Module):
    def __init__(
        ...
        self.positional_embedding = SinusoidalPositionalEmbedding(
            self.max_source_positions, d_model
        )
    ...
    def forward(self, input_features, input_length, output_hidden_states=False):
        ...
        bsz, tgt_len, _ = hidden_states.size()
        pos_embed = self.positional_embedding(tgt_len)
...
class ISTFT(nn.Module):
    ...
    def forward(self, spec: torch.Tensor) -> torch.Tensor:
        ...
        if not (window_envelope > 1e-11).all():
            raise ValueError(
                "Window envelope contains near-zero values leading to instability"
            )
...
class VectorQuantize(nn.Module):
    ...
    def forward(self, z):
        ...
        z_q_emb = F.embedding(indices, self.codebook.float()).transpose(
            1, 2
        )  # Get embeddings without projection
        commit_loss = (
            F.mse_loss(z_e, z_q_emb.detach(), reduction="none").mean([1, 2])
            * self.commitment
        )
...
class XYTokenizerPreTrainedModel(PreTrainedAudioTokenizerBase):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(
            module, (XYTokenizerEncoder, XYTokenizerDecoder, XYTokenizerTransformer)
        ):
...
class XYTokenizer(XYTokenizerPreTrainedModel):
    def __init__(self, config: XYTokenizerConfig):
        ...
        self.feature_extractor = XYTokenizerFeatureExtractor(
            **params["feature_extractor_kwargs"]
        )
        ...
        self.input_sampling_rate = getattr(
            config, "input_sampling_rate", getattr(config, "input_sample_rate", 16000)
        )
        self.sampling_rate = getattr(
            config, "sampling_rate", getattr(config, "output_sample_rate", 16000)
        )
    ...
    def scale_window_size(self, boundaries, scaling_factor):
        ...
        for left_boundary, right_boundary in boundaries:
            ...
            scaling_boundaries.append(
                slice(scaling_left_boundary, scaling_right_boundary)
            )
    ...
    def decode(
        ...
        if not isinstance(audio_codes, (torch.Tensor, XYTokenizerEncoderOutput)):
            raise TypeError(
                "only accept `torch.Tensor` or `XYTokenizerEncoderOutput` for `codec.decode()` function"
            )
    ...
    def _decode(
        ...
        return XYTokenizerDecoderOutput(
    ...
    def forward(
        ...
        if attention_mask is None:
            ...
            attention_mask = torch.ones(
                batch_size, seq_len, device=input_values.device, dtype=torch.long
            )

        features = BatchFeature(
            {"input_features": input_values, "attention_mask": attention_mask}
        )
