...
def pad_sequence(sequence, padding_size, padding_value=0.0):
    ...
    if padding_size > 0:
        return np.pad(
            sequence, (0, padding_size), mode="constant", constant_values=padding_value
        )
    return sequence
...
class ExtractorIterator:
    ...
    def __iter__(self):
        ...
        for i, sample in enumerate(self.data):
            sample_chunks, sample_lengths, sample_seq_no = self.chunk_and_pad_view(
                sample, i
            )
            ...
            while processed_in_sample < len(sample_chunks):
                ...
                # Fill data
                wav_array[start_idx_batch:end_idx_batch] = sample_chunks[
                ...
                if batch_num == self.batch_size:
                    ...
                    for xi, (_, right) in enumerate(input_lengths):
                        if right == right_boundary and np.any(
                            wav_array[xi, :, right:] != 0
                        ):
    ...
    def get_windows_boundaries(self, num_chunks, seq_len):
        ...
        output_lengths[-1] = (
            output_lengths[-1][0],
            seq_len - self.duration_size * (num_chunks - 1),
        )
